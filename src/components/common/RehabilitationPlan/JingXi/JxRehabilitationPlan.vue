  ...
  <div class="main-content">
    <div class="plan-category">
      <a-radio-group @change="handlePlanChange" default-value="日计划">
        ...
        <a-radio-button value="维持">维持计划</a-radio-button>
      </a-radio-group>
    ...
    <div class="courseTable">
      <div v-if="coursePlanCategory==='日计划'">
        <jx-daily-plan
          v-if="isCourseHave"
          :planDetail="JxDailyPlanData"
          :courseDetail="jxCourseDetail"
          :courseSchedule="daysCourseSchedule"
          @addDailyCourse="addDailyCourse"
          @editDailyCourse="editDailyCourse"/>
        <h2 v-else>今天没有精细课</h2>
      </div>
      <jx-monthly-plan
        v-else-if="coursePlanCategory==='月计划'"
        :nameid="nameid"
        :planDetail="JxMonthlyPlanData"
        :courseDetail="jxCourseDetail"
        :courseSchedule="courseSchedule"
        @addMonthlyCourse="addMonthlyCourse"/>
      <jx-maintain v-else-if="coursePlanCategory==='维持'" :planDetail="JxMaintainData"/>
    </div>
  </div>
</template>

<script>
import JxDailyPlan from './JxDailyPlan.vue'
import JxMonthlyPlan from './JxMonthlyPlan.vue'
import JxMaintain from './JxMaintain.vue'
...
import { getJxContentData } from 'network/getCourseContentData.js'
import { getWhichTimeHaveCourse } from 'network/getWhichTimeHaveCourse.js'
// import { JxDailyPlanData, JxMonthlyPlanData, JxMaintainData } from 'someData/index.js'
  ...
  methods: {
    ...
    isCourseHaveToday (values) {
      isCourseHaveToday(values).then(res => {
        ...
        this.JxDailyPlanData = res.content
        this.daysCourseSchedule = res.time
      })
    },
    getJxContentData () {
      getJxContentData().then(res => {
        this.jxCourseDetail = res
        console.log(res)
      })
    },
