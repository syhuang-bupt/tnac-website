  ...
  <div class="main-content">
    ...
    <div class="courseTable">
      <div v-if="coursePlanCategory==='日计划'">
        <gx-daily-plan
          v-if="isCourseHave"
          :planDetail="GxDailyPlanData"
          :courseDetail="gxCourseDetail"
          :courseSchedule="daysCourseSchedule"
          @addDailyCourse="addDailyCourse"
          @editDailyCourse="editDailyCourse"/>
        <h2 v-else>今天没有个训课</h2>
      </div>
      <gx-monthly-plan
        v-else-if="coursePlanCategory==='月计划'"
        :nameid="nameid"
        :planDetail="GxMonthlyPlanData"
        :courseDetail="gxCourseDetail"
        :courseSchedule="courseSchedule"
        @addMonthlyCourse="addMonthlyCourse"/>
      <gx-maintain v-else-if="coursePlanCategory==='维持'"/>
    </div>
  </div>
</template>

<script>
import GxDailyPlan from './GxDailyPlan.vue'
import GxMonthlyPlan from './GxMonthlyPlan.vue'
import GxMaintain from './GxMaintain.vue'
// import { GxDailyPlanData, GxMonthlyPlanData, GxMaintainData } from 'someData/index.js'
import { editCourseDetail } from 'network/editData.js'
import { getGxContentData } from 'network/getCourseContentData.js'
  ...
  methods: {
    ...
    isCourseHaveToday (values) {
      isCourseHaveToday(values).then(res => {
        ...
        this.GxDailyPlanData = res.content
        this.daysCourseSchedule = res.time
      })
    },
    getGxContentData () {
      getGxContentData().then(res => {
        this.gxCourseDetail = res
        console.log(res)
      })
    },
