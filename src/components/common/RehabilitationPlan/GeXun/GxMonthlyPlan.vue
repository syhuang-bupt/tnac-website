<template>
  <div class="table-content">
    <a-table :columns="columns" :data-source="tableData" bordered :pagination="pagination">
      ...
      <span slot="startTime" class="title-style">课程时间</span>
      <span slot="therapist" class="title-style">康复师</span>
      <span slot="courseProps" class="title-style">课程属性</span>
    ...
    <a-modal
      ...
      <a-form :form="form" :label-col="{ span: 6 }" :wrapper-col="{ span: 12 }">
        ...
        <a-form-item label="课程属性" v-if="editDefaultData.dateProps==='工作日'">
          <a-select
            v-decorator="['courseProps', {initialValue:editDefaultData.courseProps},{ rules: [{ required: true, message: '请选择课程属性!' }] },]"
            placeholder="课程属性"
          >
            <a-select-option v-for="(item,index) in coursePropsArray" :value="item" :key="index">
              {{ item }}
            </a-select-option>
          </a-select>
        </a-form-item>
...
const columns = [
  ...
  {
    dataIndex: 'courseProps',
    key: 'courseProps',
    align: 'center',
    slots: { title: 'courseProps' }
  },
  {
    dataIndex: 'duration',
    key: 'duration',
...
export default {
  ...
  data () {
    return {
      ...
      courseDateArray: [],
      courseTimeArray: [],
      coursePropsArray: ['月计划', '日维持', '周维持', '双周维持'],
      editDefaultData: { dateProps: '', startTime: '', courseContent: [], courseProps: '', duration: [] },
  ...
  methods: {
    ...
    Edit (record) {
      ...
      this.editDefaultData.courseContent[2] = record.taskObjectives
      this.editDefaultData.courseProps = record.courseProps
