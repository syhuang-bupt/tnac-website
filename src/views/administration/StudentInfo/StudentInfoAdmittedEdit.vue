...
<script>
import { getAdmittedStudentData, editAdmittedStudent } from 'network/editData.js'
...
export default {
  ...
  created () {
    ...
    this.getAdmittedStudentData(this.nameid)
  },
  methods: {
    moment,
    getAdmittedStudentData (values) {
      getAdmittedStudentData(values).then(res => {
          console.log(res)
          this.originInfo = res
    ...
    editAdmittedStudent (values) {
      editAdmittedStudent(values).then(res => {
          console.log(res)
      })
    },
    handleSubmit (e) {
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          this.editAdmittedStudent(values)
          setTimeout(() => {
            this.$router.push('/administration/studentmanage/studentinfo/admitted')
          }, 1000)
        }
      })
    },
