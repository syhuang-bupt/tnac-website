...
<script>
import { addAdmittedStudent } from 'network/addNewStudentOrStaff.js'
...
export default {
  ...
  methods: {
    moment,
    addAdmittedStudent (values) {
      addAdmittedStudent(values).then(res => {
          console.log(res)
      })
    },
    ...
    handleSubmit (e) {
      ...
      this.form.validateFields((err, formValues) => {
        if (!err) {
          ...
          this.addAdmittedStudent(values)
          setTimeout(() => {
