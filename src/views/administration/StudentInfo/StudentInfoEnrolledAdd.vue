    ...
    <div class="main-content">
      ...
      <a-form
        ...
        <a-form-item label="监护人电话" has-feedback>
          <a-input class="form-item-width" v-decorator="['guardianPhone',{rules: [{ required: true, message: '请输入监护人电话，只能输入数字！', pattern: new RegExp(/^[0-9]\d*$/, 'g'), whitespace: true, }],},]"/>
        </a-form-item>
        ...
        <a-form-item label="缴费情况">
          <a-switch
            ...
            v-decorator="['payState', { defaultChecked: true, valuePropName: 'checked' }]" />
        </a-form-item>

        <a-form-item label="缴费有效期至" v-if="isPayDataShow">
          <a-date-picker v-decorator="['payEffectiveDate', {rules: [{ type: 'object', required: true, message: '请选择时间!' }]}]" />
        </a-form-item>
...
<script>
import { addEnrolledStudent } from 'network/addNewStudentOrStaff.js'
...
export default {
  ...
  methods: {
    moment,
    addEnrolledStudent (values) {
      addEnrolledStudent(values).then(res => {
          console.log(res)
      })
    },
    normFile (e) {
    ...
    handleSubmit (e) {
      ...
      this.form.validateFields((err, formValues) => {
        if (!err) {
          ...
          this.addEnrolledStudent(values)
          setTimeout(() => {
