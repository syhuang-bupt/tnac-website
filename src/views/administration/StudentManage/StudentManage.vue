<template>
  <page-header-wrapper :title="false" >
    <div class="main-content">
      ...
      <a-table :data-source="studentManageData" :columns="columns" class="stu-table" bordered>
        ...
        <span slot="className" class="title-style">班级</span>
        <span slot="supervision" class="title-style">督导</span>
        <span slot="paymentState" class="title-style">缴费情况</span>
        <span slot="attendanceState" class="title-style">出勤状态</span>
        <span slot="attendanceData" class="title-style">出勤数据</span>
        <span slot="paymentState" slot-scope="paymentState"><a-badge :color="paymentState==='未缴费'?'red':'green'"/>{{ paymentState }}</span>
        <span slot="attendanceState" slot-scope="attendanceState,record">
          <a-switch
            checked-children="出勤"
            un-checked-children="缺席"
            :defaultChecked="attendanceState"
            @change="attendanceStateChange(record)"/>
        </span>
...
import { getStudentDailyManage } from 'network/getStaffOrStudentData.js'
import { postStudentManageData } from 'network/manageStaffOrStudent.js'
// import { studentManageData } from 'someData/index.js'
export default {
  name: 'StudentManage',
  components: {
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          key: 'supervision',
          align: 'center',
          slots: { title: 'supervision' }
        },
        {
          dataIndex: 'paymentState',
          key: 'paymentState',
          align: 'center',
          slots: { title: 'paymentState' },
  ...
  created () {
    this.getStudentDailyManage()
  },
  methods: {
    getStudentDailyManage () {
      getStudentDailyManage().then(res => {
        console.log(res)
        this.studentManageData = res
      })
    },
    postStudentManageData (values) {
      postStudentManageData(values).then(res => {
        console.log(values)
        console.log(res)
      })
    },
    ...
    attendanceStateChange (record) {
      record.attendanceState = !record.attendanceState
      const values = {
        name: record.name,
        attendanceState: record.attendanceState
      }
      this.postStudentManageData(values)
    }
  }
}
</script>
