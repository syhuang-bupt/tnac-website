    ...
    <div class="main-content">
      ...
      <a-form
        ...
        <a-form-item label="擅长领域">
          <a-select
            v-decorator="['goodDomain',{ initialValue: originInfo.goodDomain },{rules: [{ required: true, message: '请选择评估师的擅长领域!', type: 'array' },],},]"
            mode="multiple"
...
<script>
import { getAssessorData, editAssessor } from 'network/editData.js'
...
export default {
  ...
  created () {
    ...
    this.getAssessorData(this.nameid)
  },
  methods: {
    moment,
    getAssessorData (values) {
      getAssessorData(values).then(res => {
          console.log(res)
          this.originInfo = res
      })
    },
    editAssessor (values) {
      editAssessor(values).then(res => {
          console.log(res)
      })
    },
    normFile (e) {
