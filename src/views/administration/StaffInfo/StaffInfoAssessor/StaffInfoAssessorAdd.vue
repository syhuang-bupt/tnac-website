    ...
    <div class="main-content">
      ...
      <a-form
        ...
        <a-form-item label="电话" has-feedback>
          <a-input v-decorator="['phoneNumber',{rules: [{ required: true, message: '请输入电话号码，只能输入数字！', pattern: new RegExp(/^[1-9]\d*$/, 'g'), whitespace: true, }],},]"/>
        </a-form-item>

        <a-form-item label="擅长领域">
          <a-select
            v-decorator="['goodDomain',{rules: [{ required: true, message: '请选择评估师的擅长领域!', type: 'array' },],},]"
            mode="multiple"
            placeholder="评估师擅长领域，可多选">
        ...
        <a-form-item label="星级">
          <a-rate v-decorator="['levelStar', { initialValue: 3 }]" />
        </a-form-item>

        <a-form-item label="住址" has-feedback>
          <a-input v-decorator="['address',{rules: [{ required: true, message: '请输入住址！', whitespace: true }],},]"/>
        </a-form-item>
...
import { addAssessor } from 'network/addNewStudentOrStaff.js'
export default {
  data () {
    return {
      uploadURLStaff,
    ...
    handleSubmit (e) {
      ...
      this.form.validateFields((err, formValues) => {
        if (!err) {
          ...
          this.addAssessor(values)
          setTimeout(() => {
            this.$router.push('/administration/stuffmanage/stuffinfo/assessor')
          }, 1000)
        }
      })
    },
