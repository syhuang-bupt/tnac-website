<template>
  <page-header-wrapper :title="false">
    <div class="main-content">
      ...
      <a-table bordered :data-source="therapistsInfoData" :columns="columns" class="therapist-table">
        ...
        <span slot="goodDomain" slot-scope="goodDomain">
          <a-tag
            ...
            :key="tag"
            :color="tag === '个训' ? 'volcano' : tag === '集体' ? 'geekblue' : tag === '语言' ? 'green' : tag === '感统' ? 'purple' : 'blue'">
            {{ tag.toUpperCase() }}
          </a-tag>
        </span>
        ...
        <template slot="operation" slot-scope="text, record">
          ...
          <a-popconfirm
            v-if="therapistsInfoData.length"
            title="确定删除吗?"
...
<script>
import { deleteTherapists } from 'network/deleteData.js'
import { getAllTherapistsData } from 'network/getStaffOrStudentData.js'
// import { therapistsInfoData } from 'someData/index.js'
export default {
  ...
  created () {
    this.getAllTherapistsData()
  },
  methods: {
    getAllTherapistsData () {
      getAllTherapistsData().then(res => {
        console.log(res)
        this.therapistsInfoData = res
      })
    },
    deleteTherapists (values) {
      console.log(values)
      deleteTherapists(values).then(res => {
        console.log(res)
        if (res) {
          const therapistsInfoData = [...this.therapistsInfoData]
          this.therapistsInfoData = therapistsInfoData.filter(item => item.name !== values)
        } else {
          this.$message.warning('该康复师有安排课程未处理，请处理后再删除！')
        }
      })
    },
    ...
    onDelete (name) {
      this.deleteTherapists(name)
    },
    editTherapistInfo (name) {
      this.$router.push('/administration/stuffmanage/stuffinfo/therapists/edit/' + name)
    },
    handleAdd () {
      this.$router.push('/administration/stuffmanage/stuffinfo/therapists/add')
    }
  }
}
</script>
