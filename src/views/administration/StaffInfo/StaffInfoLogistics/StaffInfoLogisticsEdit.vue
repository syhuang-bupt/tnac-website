  ...
  <page-header-wrapper :title="false">
    <div class="main-content">
      ...
      <a-form
        ...
        <a-form-item label="部门" has-feedback>
          <a-select
            v-decorator="['department',{ initialValue:originInfo.department },{ rules: [{ required: true, message: '请选择部门!' }] },]">
...
<script>
import { getLogisticsData, editLogistics } from 'network/editData.js'
...
export default {
  ...
  created () {
    ...
    this.getLogisticsData(this.nameid)
  },
  methods: {
    moment,
    getLogisticsData (values) {
      getLogisticsData(values).then(res => {
          console.log(res)
          this.originInfo = res
      })
    },
    editLogistics (values) {
      editLogistics(values).then(res => {
          console.log(res)
      })
    },
    normFile (e) {
    ...
    handleSubmit (e) {
      ...
      this.form.validateFields((err, formValues) => {
        if (!err) {
          ...
          this.editLogistics(values)
          setTimeout(() => {
            this.$router.push('/administration/stuffmanage/stuffinfo/logistics')
          }, 1000)
        }
      })
    },
