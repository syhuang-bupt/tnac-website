<template>
  <page-header-wrapper :title="false">
    <div class="main-content">
      ...
      <a-table bordered :data-source="otherDepartmentInfoData" :columns="columns" class="therapist-table">
        ...
        <span slot="department" class="title-style">部门</span>
        ...
        <template slot="operation" slot-scope="text, record">
          ...
          <a-popconfirm
            v-if="otherDepartmentInfoData.length"
            title="确定删除吗?"
...
<script>
import { deleteLogistics } from 'network/deleteData.js'
import { getAllLogisticsData } from 'network/getStaffOrStudentData.js'
// import { otherDepartmentInfoData } from 'someData/index.js'
export default {
  name: 'StaffInfoLogistics',
  components: {
  ...
  created () {
    this.getAllLogisticsData()
  },
  methods: {
    getAllLogisticsData () {
      getAllLogisticsData().then(res => {
        console.log(res)
        this.otherDepartmentInfoData = res
      })
    },
    deleteLogistics (values) {
      deleteLogistics(values).then(res => {
        console.log(res)
      })
    },
    ...
    onDelete (name) {
      this.deleteLogistics(name)
      const otherDepartmentInfoData = [...this.otherDepartmentInfoData]
      this.otherDepartmentInfoData = otherDepartmentInfoData.filter(item => item.name !== name)
    },
    editTherapistInfo (name) {
      this.$router.push('/administration/stuffmanage/stuffinfo/logistics/edit/' + name)
    },
    handleAdd () {
      this.$router.push('/administration/stuffmanage/stuffinfo/logistics/add')
    }
  }
}
</script>
