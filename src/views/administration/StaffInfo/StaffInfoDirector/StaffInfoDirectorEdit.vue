...
<script>
import { getDirectorData, editDirector } from 'network/editData.js'
...
export default {
  ...
  created () {
    ...
    this.getDirectorData(this.nameid)
  },
  methods: {
    moment,
    getDirectorData (values) {
      getDirectorData(values).then(res => {
          console.log(res)
          this.originInfo = res
      })
    },
    editDirector (values) {
      editDirector(values).then(res => {
          console.log(res)
      })
    },
    normFile (e) {
