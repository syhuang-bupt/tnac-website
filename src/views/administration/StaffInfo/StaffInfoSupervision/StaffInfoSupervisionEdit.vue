...
<script>
import { getSupervisionData, editSupervision } from 'network/editData.js'
...
export default {
  ...
  created () {
    ...
    this.getSupervisionData(this.nameid)
  },
  methods: {
    moment,
    getSupervisionData (values) {
      getSupervisionData(values).then(res => {
          console.log(res)
          this.originInfo = res
      })
    },
    editSupervision (values) {
      editSupervision(values).then(res => {
          console.log(res)
      })
    },
    normFile (e) {
    ...
    handleSubmit (e) {
      ...
      this.form.validateFields((err, formValues) => {
        if (!err) {
          ...
          this.editSupervision(values)
          setTimeout(() => {
            this.$router.push('/administration/stuffmanage/stuffinfo/supervision')
          }, 1000)
        }
      })
    },
