  ...
  <page-header-wrapper :title="false">
    <div class="content-main" ref="maincontentheight">
      ...
      <div class="right-content">
        ...
        >
          <assessment-report v-if="noTitleKey === 'AssessmentReport'" style="min-height:408px"/>
          <iEP-plan v-if="noTitleKey === 'IEPPlan'" style="min-height:408px"/>
        </a-card>
      </div>
    </div>
  </page-header-wrapper>
...
import { getBusinessEvaluationStudentDetail } from 'network/businessStudentData.js'
export default {
  ...
  data () {
      return {
        ...
        noTitleKey: 'AssessmentReport'
    }
  },
  created () {
    this.nameid = this.$route.params.nameid
    // 取消这一行的注释即可
    this.getBusinessEvaluationStudentDetail(this.nameid)
  },
  methods: {
    handleTabChange (key, type) {
      this[type] = key
    },
    getBusinessEvaluationStudentDetail (values) {
      getBusinessEvaluationStudentDetail(values).then(res => {
